...
public class LinearProbing{
  ...
  //hash function to return the index
  ...
  //depends on number of calls used if >.75 then create a new harsh table
  public double getLoadFactor() {
    double loadFactor = usedCellNumber * 1.0/hashTable.length;
    return loadFactor;
  }

// Rehash the keys in case the load factor is bigger than .75
  public void rehashKeys(String word) {
    usedCellNumber = 0;
    ArrayList<String> data = new ArrayList<String>();
    for (String s : hashTable) {
      if (s != null) {
        data.add(s);//add all data from previous table to new data structure
      }
    }
    data.add(word); //add new word to this new DS
